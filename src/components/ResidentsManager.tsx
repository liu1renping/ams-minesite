...
export function ResidentsManager({ residents }: { residents: ResidentRecord[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return residents.filter((r) =>
      [r.employeeId, r.firstName, r.lastName, r.company, r.role, r.phone, r.email]
        .join(" ")
        .toLowerCase()
        .includes(q),
    );
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    const res = await fetch(isEditing ? `/api/residents/${editingId}` : "/api/residents", {
  ...
  async function onDelete(id: string, label: string) {
    if (!window.confirm(`Delete resident ${label}? This cannot be undone.`)) return;
                  ...
                  <td className="px-2 py-3">
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className="rounded-md bg-sky-700/70 px-2.5 py-1 text-xs text-white hover:bg-sky-600 disabled:opacity-50"
                      >
                        Edit
                      </button>
                      <button
